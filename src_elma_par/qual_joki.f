...
C
      COMMON /IMP/ MKC,LAMB(NDIM4*NDIM4),
     *       NUHI(NZBMAX*NZBMAX),
...
C
      COMMON /INDY/ LAHI(NZBMAX*NZBMAX)
      dimension iindy(lindy)
      equivalence (iindy(1),lahi)

      COMMON /BIGA/ F(NDIM,NDIM) 
      dimension rbiga(lbiga)
      equivalence (rbiga(1),f)
      COMMON /BIGB/ DM(NDIM,NDIM,NDIM2)
      dimension rbigb(lbigb)
      equivalence (rbigb(1),dm)
...
C     Anzahl der Prozessoren
      ...
      OPEN(UNIT=17,FILE='QUAALT',STATUS='UNKNOWN',FORM='UNFORMATTED')
C
      INPUT=5
      NOUT=6
...
C
      ...
      write(nout,*) NZF,NZT,NZV,(NREG(K),K=1,NZOPOB), INDEPO
...
c #ifdef  PC_FORTRAN
c c        OPEN(UNIT=6,FILE='OUTPUT', ACCESS='APPEND')
c #endif
c #ifdef  CRAY_FORTRAN
c c        OPEN(UNIT=6,FILE='OUTPUT', POSITION='APPEND')
c #endif
...
c      write(nout,*) '<> NTE,NTE,NTE',NTE,NTE,NTE
  ...
  425 CONTINUE
      ...
      DO 922   MTE = 1,NTE
      READ(NBAND2) NT1(MTE),IMQ,(LCALT(I),I=1,NZT)
     *  ,           (IMV(I),UM(I),I=1,IMQ)
c      write(nout,*) '<>  ',NT1(MTE),IMQ,(LCALT(I),I=1,NZT)
c     *  ,           (IMV(I),UM(I),I=1,IMQ)
      DO 11 J=1,NR
      DO 11 I=1,NL
      U(J,M,MTE)= 0.
11    CONTINUE
...
c -- cheap talk
      ...
      WRITE (NOUT,838) NT1(MTE), (LC(K,MTE),K=1,NZT)
...
c schon alle beschaeftigt?
      ...
      if(narbeit .ge. (nproc-1) ) then
         write(nout,*)'waiting for the lazy slaves...'
c        alle beschaeftigt, erstmal auf Ergebnisse warten
     ...
     *        MPI_COMM_WORLD,istatus,ierr)
         call MPI_RECV(rbiga,lbiga,MPI_DOUBLE_PRECISION,ngerechnet,0,
     *        MPI_COMM_WORLD,istatus,ierr)
         call MPI_RECV(rbigb,lbigb,MPI_DOUBLE_PRECISION,ngerechnet,0,
     *        MPI_COMM_WORLD,istatus,ierr)
         ...
         write(27) (rbiga(lb),lb=1,lbiga)
         write(27) (rbigb(lb),lb=1,lbigb)

         write(nout,*) 'shape(INDY):',shape(iindy)
         write(nout,*) 'shape(BIGa):',shape(rbiga)
         write(nout,*) 'shape(BIGb):',shape(rbigb)

         WRITE(nout,*) 'nteilalt,icount,index(nteilalt)',
     *          nteilalt,icount,index(nteilalt)
         write(nout,*) '1...',lindy,':',(iindy(lb),lb=14399,14405)
         write(nout,*) '1...',lbiga,':',(rbiga(lb),lb=1,3)
         write(nout,*) '1...',lbigb,':',(rbigb(lb),lb=14399,14405)
...
c     hier muss die Schleife zum Aufsammeln der restlichen Ergebnisse hin
     ...
     *     MPI_COMM_WORLD,istatus,ierr)
         ...
         WRITE(nout,*) 'nteilalt,icount,index(nteilalt)',
     *          nteilalt,icount,index(nteilalt)
         write(nout,*) 'nteil,index(1)...index(nteil):',
     *                  nteil,(index(iy),iy=1,nteil)
         write(nout,*) '1...',lindy,':',(iindy(lb),lb=1,3)
         write(nout,*) '1...',lbiga,':',(rbiga(lb),lb=1,3)
         write(nout,*) '1...',lbigb,':',(rbigb(lb),lb=1,3)
...
c        close(unit=15)
     ...
     *     MPI_COMM_WORLD,ierr)
      call MPI_SEND(rbiga,lbiga,MPI_DOUBLE_PRECISION,0,0,
     *     MPI_COMM_WORLD,ierr)
      call MPI_SEND(rbigb,lbigb,MPI_DOUBLE_PRECISION,0,0,
     *     MPI_COMM_WORLD,ierr)
...
C      JPR=(KPR-1)*NDIM3+KLR
      ...
      A = TS * U(KSL,KSR,MTE)*COF(LPAR,NN,MFR)
...
C
      COMMON /LUR/ IENT(NZOPLU), NDIM1H, NBAND3
C
C      COMMON /INDY/ LAHI(NZBMAX*NZBMAX)
C      COMMON /BIGA/ F(NDIM,NDIM)
C      COMMON /BIGB/ DM(NDIM,NDIM,NDIM2)
C
C  MULPOL common blocks -----------------------------------
      COMMON /LUP2/ NDIM4H,MUL,LINDEX(NDIM1),LINT(NZAVMA,NDIM1),
     *              LSUM(NDIM1), NDIM2H
...
C      EQUIVALENCE (WERT(1,1,1),WERTT(1,1)), (DM(1,1,1),DMM(1,1))
...
C     ADREESRECHNUNG UND BESTIMMUNG VON DMM
      ...
      DO 100 M=1,NSH
       ...
       MHI = NUHI(M)
       LAHI(MHI) = LL1
c       if (LL1.ne.0) write(nout,*)'begri:',MHI,LAHI(MHI)
       DO 100 K = 1,LL1
        C = SH(M) * WERTT(I2,K)
