...
C      NBAND3 GESCHRIEBEN
      ...
      REWIND NBAND3
      WRITE (nout,*) '(nband2) MKC=',MKC,':',NNN, NNN, NNN
      WRITE(NBAND2)    NNN,NNN,NNN
...
C     NICHT KOPIERTE OPERATOREN
      ...
      WRITE (NBAND7) N1, (NZD(I6),I6=1,NZT),
     *       ((UU(KFL,KFR),KFL=1,MZGL),KFR=1,MZGR)
      GOTO 811
  804 CONTINUE
C     KOPIERTE OPERATOREN
      READ (NBAND7) N1, (NZD(I6),I6=1,NZT),
     *       ((UU(KFL,KFR),KFL=1,MZGL),KFR=1,MZGR)
      IF (NNN.EQ.0) GOTO 700
      GOTO 811
  806 CONTINUE
C     NORM
      READ (NBAND3) N1, (NZD(I6),I6=1,NZT)
  811 WRITE(nout,*) '<>',N1, (NZD(I6),I6=1,NZT) ,
...
C      DC NACHEINANDER ERFOLGEN
      ANSP(J)=ANSP(J)+REAL(NP(5))*FAK*REAL(IFT)
...
2125  C(L,I)=0.
   ...
   10 CONTINUE
      ...
      L4 = L1 + L3
      DO 17   M = 1,L1
   17 SVEC(M,I) = -1./ REAL(L1)
      DO 16   M = L2,L4
   16 SVEC(M,I) =  1./ REAL(L3)
   14 IF(K1.LE.LL) GOTO 24
      ...
      L4 = L1 + L3
      DO 22   M = L5,L1
   22 SVEC(M,I) =-1./ REAL(L1-L5+1)
      DO 23   M = L2,L4
   23 SVEC(M,I) = 1./ REAL(L3)
   21 CONTINUE
   24  L4 = L5 - 1
      DO 26   M  = 1,L4
      SVEC(M,NZV) = -1./ REAL(L4)
   26 SVEC(M,NZT) = 1./ REAL(NZT)
      DO 27   M = L5,NZT
      SVEC(M,NZV) = 1./ REAL(NZT-L4)
   27 SVEC(M,NZT) = 1./ REAL(NZT)
      DO 60   K = 1,NZT
      A = .0
      DO 61   L = 1,NZT
   61 A = A + SVEC(L,K)**2
