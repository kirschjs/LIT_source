...
C
      COMMON/ORD/KZAHL, JQ(NDIMOR), EPO(NDIMOR),
...
C
C
      READ (5,1000) NBAND1,NBAND2,LAUS,KAUS,MKAUS,LALL
...
C
     ...
     1        (IENT(MH), MH=1,NZOPER), (NZPO(MH),MH=1,NZF)
...
C     ************************************************************
C
C     ANFANG DER BERECHNUNG DER MATRIXELEMENTE
C     ========================================
C
...
145   CONTINUE
      F10=REAL(IH)
      F9=SQRT(REAL(MUL)/REAL(MUL+1))*F10*(-1)
...
187        F=F*F9*SQRT(REAL(MUL)/REAL(2*MUL+1))
           GOTO 194
188        STOP 6
189        F=F*F9*SQRT(REAL(MUL+1)/REAL(2*MUL+1))*(-1)
           GOTO 194
190        CONTINUE
...
C          KORREKTUROPERATOR ZUM SIEGERTOPERATOR
           LOP=MUL+1
           F=F*F9*(-1)*SQRT((2.*REAL(MUL)+1.)/REAL(MUL))
C
...
217          L1=2*LOP
     ...
     *              2.*(1-ABS(IPOLAR)))
             IF (ABS(FPOLAR).LT.1E-17) GOTO 470
C
C
...
C     NZLIK : GESAMTANZAHL ALLER LI-KOMBINATIONEN FUER ALLE
C             DREHIMPULSSTRUKTUREN
      COMMON /LWERTE/ N, L0, M0, LEHILF(NZLMAX),
     *                MEHILF(NZLMAX), MIHILF(NZLMAX),
     *                LI(NZLMAX), MI(NZLMAX,NDIMC), CH
...
C
      F1=F7(1)*MINUS1(L0+M0)/SQRT(REAL(2*L0+1))
      LESUM=LE(1)
...
C      IMPLICIT NONE
C
      INTEGER   NZCMAX,NDIMC,LI,MIMAX,MIMIN,MPFEIL,M,
     +           MOBEN,MUNTEN,MRECHT,MLINKS,NZLI,I,J,
     +           K,KUI,KOI,KUJ,KOJ,NZMIK,MHILF,LE,ME,
     +           LDUMMY,MDUMMY,NZLMAX
      REAL*8 CH
       INCLUDE 'par/LUISE'
      PARAMETER (NZLMAX=3*2*(NZCMAX-1))
     ...
     +           MPFEIL(NZLMAX),ME(NZLMAX),MDUMMY(NZLMAX)
      COMMON/LWERTE/NZLI,LDUMMY,M,LE,ME,MDUMMY,LI,MI,CH
C
...
C
      INTEGER N, L0, M0, L, M, LKOP, MKOP, K, K1, I, NZCMAX,
     *        NDIMC, NZLMAX, LALL, L1, L2, L3, NKOP, LW1, LW2,
     *        LW3, MW1, MW2
      REAL*8 Q, GINT, CH
      INCLUDE 'par/LUISE'
      PARAMETER (NZLMAX=3*(2*(NZCMAX-1)))
      COMMON /LWERTE/ N, L0, M0, L1(NZLMAX),
     *                L2(NZLMAX), M(NZLMAX),
     *                L(NZLMAX), L3(NZLMAX,NDIMC), CH
C
...
C      IMPLICIT NONE
     ...
     *             INDEX(NDIMOR)
      ...
      PRINT 1000
      DO 10 KH=1,KZAHL
       write(*,*) '<>',JQ(KH),INDPO(KH),EPO(KH),INDEX(KH),
     *             (MVK(IH,KH),IH=1,IQM)
10    CONTINUE
20    CONTINUE
