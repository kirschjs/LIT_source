...
C
     ...
     *        RPAR(MZPARM,NZFMAX), VR(NZTMAX,NZCMAX), WR(NZTMAX)
C
...
C     EINLESEN DER FUNKTIONSEIGENSCHAFTEN UND PARAMETER                 
      ...
      IF(MOBAUS.GT.0) WRITE(NOUT,*)' NZF',NZF,' NZT',NZT,
     +        (NREG(K),K=1,NZOPOB)
   ...
   93 CONTINUE                                                          
      ...
      IF(MOBAUS.GT.0) WRITE(NOUT,*)' ZERLEGUNG',K,' CLUSTER',NZC(K),
     *       NOL(K)
      M = MZG(K)                                                        
...
248   FORMAT(' VON LUELMA,ZAHL DER ZERLEGUNGEN',I3,'  MULTIPOLARITAET',
     1I3,'     OPERATOREN',4I4,/,('     ZAHL DER DREHIMPULSE',I4,
...
2011  FORMAT(////18H BERECHNET WERDEN )                                 
      WRITE (NOUT,2011)                                                 
      IF(LREG(1).GT.0) WRITE(NOUT,611) MUL
      IF(LREG(2).GT.0) WRITE(NOUT,612) MUL
      IF(LREG(3).GT.0) WRITE(NOUT,613) MUL
      IF(LREG(4).GT.0) WRITE(NOUT,614) MUL
      IF(LREG(5).GT.0) WRITE(NOUT,615) MUL
      IF(LREG(6).GT.0) WRITE(NOUT,616) MUL
      IF(LREG(7).GT.0) WRITE(NOUT,617) MUL
      IF(LREG(8).GT.0) WRITE(NOUT,618) MUL
      IF(LREG(9).GT.0) WRITE(NOUT,619) MUL
      IF(LREG(10).GT.0) WRITE(NOUT,620) MUL
611   FORMAT(//' E',I2,'-PROTON-BAHN-OPERATOR')
612   FORMAT(//' E',I2,'-NEUTRON-BAHN-OPERATOR')
613   FORMAT(//' E',I2,'-PROTON-SPIN-OPERATOR')
614   FORMAT(//' E',I2,'-NEUTRON-SPIN-OPERATOR')
615   FORMAT(//' M',I2,'-PROTON-BAHN-OPERATOR')
616   FORMAT(//' M',I2,'-NEUTRON-BAHN-OPERATOR')
617   FORMAT(//' M',I2,'-PROTON-SPIN-OPERATOR')
618   FORMAT(//' M',I2,'-NEUTRON-SPIN-OPERATOR')
619   FORMAT(//' E',I2,'-R**L*YL-PROTON-OPERATOR')
620   FORMAT(//' E',I2,'-R**L*YL-NEUTRON-OPERATOR')
      REWIND NBAND1                                                     
...
3009  FORMAT(////19H ZWISCHEN ZERLEGUNG,I3,15H UND ZERLEGUNG ,I3,
     124H WIRD ZUR ZEIT GERECHNET )                                     
      ...
      NZAV=MC1+MD1+1
      IZLURE=1
      GOTO(201,201,202,202,201,201,202,202,202,202),MKC
201   NREB=1
      IZLURE=MC1*(1+ITPO(MFL))+MD1*(1+ITPO(MFR))+1
202   CONTINUE
      IF(LREG(MKC).LE.0) GOTO 3008
      GO TO (4001,4002,4003,4004,4005,4006,4007,4008,4009,4010),MKC
4001  WRITE(NOUT,611) MUL
      GO TO 3008
4002  WRITE(NOUT,612) MUL
      GO TO 3008
4003  WRITE(NOUT,613) MUL
      GO TO 3008
4004  WRITE(NOUT,614) MUL
      GO TO 3008
4005  WRITE(NOUT,615) MUL
      GO TO 3008
4006  WRITE(NOUT,616) MUL
      GO TO 3008
4007  WRITE(NOUT,617) MUL
      GOTO 3008
4008  WRITE(NOUT,618) MUL
      GOTO 3008
4009  WRITE(NOUT,619) MUL
      GOTO 3008
4010  WRITE(NOUT,620) MUL
3008  CONTINUE      
      NL = MZG(MFL)
      NR = MZG(MFR)
      IY=MOD(MKC-1,4)+1
...
82    CONTINUE
      IF(NTE(IY).LE.0 .OR. mkc.gt.4) GOTO 440
      DO 922   MTE = 1,NTE(IY)
     ...
     1  ((U(NFL,NFR,MTE,IY),NFL=  1,NL),NFR=1,NR)
      ...
      WRITE (NOUT,838)  NT1(MTE,IY),(LC(K,MTE,IY),K=1,NZT)
 838  FORMAT (12H VON OBELMA ,20I3)
      IF(MOBAUS.LT.2) GOTO 922
      WRITE (NOUT,839) ((U(NFL,NFR,MTE,IY),NFL=  1,NL),NFR=1,NR)
...
95    IF(IRHO*JRHO.LE.0) GOTO 42
  ...
  490 LC1(M)=0                                                          
      ...
      WRITE(NOUT,*) MTE,' TES OBERMATRIXELEMENT',U(1,1,MTE,IY)
1491  MM=0                                                              
      DO 491 M=1,NZT                                                    
...
512   WRITE(NBAND1) NUML,NUMR,IK1,JK1,((DM(K,L),K=M1,M2),L=N1,N2)
      ...
      IF(NAUS.LT.2) GOTO 481
       WRITE (NOUT,1021) ((DM(K,L), K=M1,M2),L=N1,N2)
...
C
      ...
      COMMON /BEG/ IZLURE
      COMMON /LUR/ IENT(NZOPLU+1),NBAND3,NBAND7
      IF(MKC.EQ.3.OR.MKC.EQ.9) REWIND NBAND7
      DO 109 I=1,IZ
      DO 109 J=1,JZ
109   WERTL(I,J)=0.
      kzhv(2,1)=0
      GOTO (443,500,442,500,443,500,442,500,442,500),MKC
442   NLUDZ=NLUDZ+1
      IENT(5)=IENT(2)
...
20    READ(NBAND3) K1ZAHL,IQM
      IF(MKC.EQ.3) WRITE(NBAND7) K1ZAHL,IQM
21    IQM=MAX0(IQM,1)
...
22    READ(NBAND3) (JQ(KW),INDPO(KW),EPO(KW),(MVK(IW,KW),IW=1,IQM),
     1 KW=1,K1ZAHL)
      IF(MKC.EQ.3) WRITE(NBAND7)(JQ(KW),INDPO(KW),EPO(KW),
...
23    CONTINUE
  ...
  443 NLUDZ=NLUDZ+1                                                     
      ...
      IF(LUPAUS.GT.0) WRITE (NOUT,837) IIZ,K2ZAHL,IQM
837   FORMAT(' RECORD VON LUELMA, IIZ,K,IQM',3I10)
      IQM=MAX0(IQM,1)                                                   
      IF(IQM.GT.NZIQMA) STOP 'NZIQMA ZU KLEIN'
      KZHV(1,IIZ)=KZAHL
      KZHV(2,IIZ)=K2ZAHL
...
144   H(II)=HX(NH)
   ...
   70 CONTINUE                                                          
      ...
      I1 = NSH1(M) + I0                                                 
      DMM(I1) = DMM(I1) + C
      IF(NAUS.GT.2) WRITE(NOUT,*)' I1,SH,WE,C,DM',I1,SH(M),WERTT(I2),C,
     *             DMM(I1)
  100 CONTINUE                                                          
  170 CONTINUE                                                          
      RETURN                                                            
...
C
     ...
     *        VZ(NZVMAX,NZVMAX), WERT(NDIM4,NDIM4)
      ...
      DO 4   LL = 1,IZ
    4 WERT(LL,LR) = .0                                                  
      GO TO(11,11,10,10,11,11,10,10,10,10),MKC
   10 NN = 1                                                            
      GO TO 12                                                          
   11 NN = NQ
12    NU = 1                                                            
