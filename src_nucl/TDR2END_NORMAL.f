...
600   IF(I.GT.0) GOTO 11
      ...
      MLAD(1,N)=MMLAD(1,J)
      MLAD(2,N)=MMLAD(2,J)
      E2K(N)=.5* FLOAT(MLAD(1,N)*MLAD(2,N))*E2R0*REDM(N)/H2M
      DO 12 M=1,3
   12 MS(M,N)=MMS(M,J)
      DO 13 M=1,5
...
C     BESTIMMUNG DER KANAELE,DIE ZUE EINEM POLYNOMKANAL GEHOEREN
      ...
      GAMMA(2)=1.
      DO 100 K=1,28
  100 GAMMA(K+2)= FLOAT(K)*GAMMA(K)/2.
...
c     IF( ABS(B1/B2).GT.. 001) GOTO 163
 ...
 661  CONTINUE
      ...
      FAKK=FLOAT(MLAD(1,K)*MLAD(2,K))*E2R0*WNORM(KP)*SQRT(REDM(K))
     1  *GAMMA(2*M2)*.5
      DO 66 M=1,MMK
      DO 66 N=1,MMK1
