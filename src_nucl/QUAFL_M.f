...
C
     ...
     *        RPAR(NZPARM,MZPARM,NZFMAX), VR(NZTMAX,NZCMAX-1),
     *        WR(NZTMAX),KPAR
C
...
C
      OPEN(UNIT=5 ,FILE='INQUA_M'  ,STATUS='OLD')
...
C      WRITE(NOUT, 1111)
1111  FORMAT('1 QUAFL_N VERSION VOM 19.10.00')
      INPUT=5
      INPOT=25
...
C     CHECK FUER LS**2 POTENTIAL TENSOR
      IF (NREG(5).EQ.0 .OR. IENT(7).EQ.0) STOP  1256
1362  IF (LREG(12).EQ.0) GOTO 1361
      IF (NREG(6).EQ.0) STOP  1362
1361  IF (LREG(13).EQ.0) GOTO 1363
      IF (NREG(7).EQ.0 .OR. IENT(6).EQ.0) STOP  1361
1363  IF (LREG(14).EQ.0) GOTO 262
      IF (NREG(8).EQ.0) STOP  1363
262   CONTINUE
...
C
 ...
 1050 FORMAT(//15H DEFINITION DER,I3,22H TEN BASISFUNKTION IST/
     ...
     3 ' SUMMATION UEBER INNERE WEITEN MIT KOEFFIZIENTEN',/,(1P20E12.4))
...
c        PRINT 50,M,NSH,K,LL1,SH(M),WERTT(I2,K),DMM(I1,K),FG
   ...
   53 FORMAT(' BEGRI: MKC, SH(M), WERTT:',
     $ 1I4,2E16.8/)   
  100 CONTINUE 
c       IF(NAUS.GT.1) WRITE(NOUT,120) M,I1,I2,SH(M),CC,DMM(I1,1),DMM(I1,2)
c 120   FORMAT(' M,I1,I2,SH,CC,DMM1,2 ',3I5,E14.6,3E20.14)
c   100 CONTINUE
  170 CONTINUE
      RETURN
   30 CONTINUE
