...
C      NZOP=29 FUER AUFGESPALTENE 3N-POTENTIALE
...
C             GELESEN H.M.H.
C
C     FOLGENDE OPERATOREN WERDEN BERECHNET (NUKLEONRECHNUNG):
C     1. NORM 
...
C
     ...
     *       IDRU, NB15, IPLO
C
      COMMON /XH/ KPB(NZBMAX),  LREG(NZOPER), NZPAQ(NZBMAX),
     *          MMASSE(2,NZBMAX), MMLAD(2,NZBMAX), 
     *          LUM(MZPARM+1,NZKMAX), MS(3,NZKMAX), NKAENT(NZKMAX),
     *          NZRHO(NZFMAX), MREG(NZOPER), KANUM(NZBMAX,KKMAX),
     *          PAQ(MZPARM,NZBMAX), E2K(NZKMAX), DN(MZPARM,MZPARM,2),
     *          OPW(NZOPER), UMKOF(NZKMAX,NZBMAX), JKK(NZBMAX)
      COMMON /YYX/ OBWERT(NZOPER+2,NZZMAX,NZZMAX),
     *          NUMK(NDIMD), UMK(NZBMAX), POTFAK(NZOPER),
     *          MUMK(NZBMAX), UNK(NZUMAX), NCOF(NZBMAX,NZKMAX)
C
      character*25  fnumber
      character*80 cquaout, qname, cdquaout, dqname
      DATA CQUAOUT /'TQUAOUT.'/, CDQUAOUT /'TDQUAOUT.'/
C
...
C
      ...
      OPEN(UNIT=6,FILE=OUTPUTout1)
      
      call getarg(3,OUTPUTout2)
      OPEN(UNIT=29,FILE=OUTPUTout2,STATUS='UNKNOWN',
     *   FORM='UNFORMATTED')
c      OPEN(UNIT=29,FILE=OUTPUTout2,STATUS='UNKNOWN',FORM='FORMATTED')
      MATOUN=29
C
...
C
 1002 FORMAT(50I3)
 1005 FORMAT(10H ES WERDEN,I5,20H KANAELE BETRACHTET //)
 ...
 1006 FORMAT(' FEHLER IN DEN EINGABEDATEN, IM KANAL',I4,' IST BV',I4,
     1       ' FALSCH')
 1007 FORMAT(//15H DEFINITION DES,I5,15H TEN KANALS IST/
 ...
 1018 FORMAT (I4,4H ) =,4(F10.4,2H /,I4,2H )),
     1   30(/,7X,4(F10.4,2H /,I4,2H ))))
...
C     FAKU(I)=REAL(I!!)
      WRITE (NOUT,1111)
 1111 FORMAT(' STREU T-ENDMAT VOM 16.5.00')	
C
      D(1)=.0
      D(2)=.0
      DO 199 I=2,99
...
C
C     FUER SMIN SIND DIESE OPEN STATEMENTS  AUF SCRATCH UMZUSTELLEN
C
      IF(IPLO.GE.0) OPEN(UNIT=9,FILE='NORMOUT',STATUS='UNKNOWN',
     1                  FORM='UNFORMATTED')
      IF(IPLO.GT.0) OPEN(UNIT=15,FILE='PLOTOUT',STATUS='UNKNOWN',
     1              FORM='UNFORMATTED')
CC    OPEN(UNIT=9,STATUS='SCRATCH',FORM='UNFORMATTED')
CC    OPEN(UNIT=12,STATUS='SCRATCH',FORM='UNFORMATTED')
CC    OPEN(UNIT=15,STATUS='SCRATCH',FORM='UNFORMATTED')
      IZWEI=0
      IDREI=0
      IF(NBAND1.GT.0)  THEN
...
113   LREG(K)=0
     ...
     1  (NZRHO(K),K=1,NZF)
      IF(IDREI.EQ.1) READ(11) NZF,(LREG(K),K=15,NZOP),NZBASV3,
     1  (NZRHO(K),K=1,NZF)
C 
...
600   IF(I.GT.0) GOTO 11
      ...
      REDM(N)=1./(1./REAL(MASSE(1,N))+1./ REAL(MASSE(2,N)))
      MLAD(1,N)=MMLAD(1,J)
      MLAD(2,N)=MMLAD(2,J)
      E2K(N)=.5* REAL(MLAD(1,N)*MLAD(2,N))*E2R0*REDM(N)/H2M
      DO 12 M=1,3
   12 MS(M,N)=MMS(M,J)
      DO 13 M=1,5
   ...
   11 IF(KEIND.LT.0) GOTO 15
     ...
     *   GOTO 16
   15 IF(IABS(NZP(N)-NZPAQ(J)).NE.0) GOTO 16
...
C       KANALDEFINITION
      J = 0
      DO 161 M=1,NZBASV
      IF (ABS(UMKOF(N,M)).LT.1.D-12) GOTO 161
      J = J + 1
      NUMK(J) = M
      UMK(J) = UMKOF(N,M)
 161  CONTINUE
...
C
 ...
 193  NCOF(L,N) = 1
  ...
  713 CONTINUE
      ...
      IF(LWERT(5,N).NE.MLWERT(5,J)) GOTO 16
      IF(IABS(MASSE(1,N)-MMASSE(1,J))+IABS(MASSE(2,N)-MMASSE(2,J))
     &.NE.0) GOTO 16
      IF(LWERT(4,N).NE.MLWERT(4,J)) GOTO 16
   84 CONTINUE
...
C
 ...
 9990    qname = cquaout(1:k-1) // fnumber(i:j)
         do k=1, 80
            if(cdquaout(k:k).eq.' ') goto 9995
         end do
 9995    dqname = cdquaout(1:k-1) // fnumber(i:j)
       CLOSE(UNIT=21,STATUS='KEEP')
       CLOSE(UNIT=31,STATUS='KEEP')
       if(izwei.eq.1)
     1 open(unit=21,file=qname, status='unknown', form='unformatted')
       if(idrei.eq.1)
     1 open(unit=31,file=dqname, status='unknown', form='unformatted')
        if(izwei.eq.1) then
           nband21= 21
           rewind nband21
        endif
...
430   CONTINUE
      DO 40 NLES=1,MLES
        READ(NBAND21) NUML,NUMR,IK1,JK1,(((DN(K,L,I),K=1,IK1),L=1,JK1),
     1 I=1,2)
...
99    FAK1 = UMKOF(KANL,NUML)*UMKOF(KANR,NUMR)
      FAK2 = UMKOF(KANL,NUMR)*UMKOF(KANR,NUML)
       IF (ABS(FAK1)+ABS(FAK2).LT.1.D-12) GOTO 142
...
C     WIRD NUR EINMAL DURCHLAUFEN 
      ...
      KK = IZQ(K) + M
C     CHECK NORM GT 0
      IF (OPWERT(MKCU,KK).GT.1.D-13) GOTO 1400
...
C
      ...
      DO 590, M=1,MMM
      DO 590, N=1,MMM
      OBWERT(1,K,L)=OBWERT(1,K,L)+
     * QQ(M,K)*QQ(N,L)*DM(M,N,2,1)/SQRT(DM(M,M,2,1)*DM(N,N,2,1))
      OBWERT(IIK,K,L)=OBWERT(IIK,K,L)+
     * QQ(M,K)*QQ(N,L)*DM(M,N,2,2)/SQRT(DM(M,M,2,1)*DM(N,N,2,1))
c      OBWERT(1,K,L)=OBWERT(1,K,L)+QQ(M,K)*QQ(N,L)*DM(M,N,2,1)
c      OBWERT(IIK,K,L)=OBWERT(IIK,K,L)+QQ(M,K)*QQ(N,L)*DM(M,N,2,2)      
590   CONTINUE
610   CONTINUE
...
C     SCHREIBEN AUF TAPE ZUR WEITERGABE
     ...
     *      EBIN(M),WNORM(M),RUHM(1,MH),RUHM(2,MH)
1102  CONTINUE
...
76    NUMK(MK)=10000.*OPWERT(1,MK)
      ...
      KK = NZQ(L)+1
      KL = NZQ(L+1)
75    WRITE (NOUT,105) (NUMK(M),M=KK,KL)
...
78    NUMK(MK)=10000.*OPWERT(2,MK)
      ...
      KK = NZQ(L)+1
      KL = NZQ(L+1)
77    WRITE (NOUT,105) (NUMK(M),M=KK,KL)
      WRITE(NOUT,305)
...
C     BESTIMMUNG DER KANAELE,DIE ZUE EINEM POLYNOMKANAL GEHOEREN
      ...
      GAMMA(2)=1.
      DO 100 K=1,28
  100 GAMMA(K+2)= REAL(K)*GAMMA(K)/2.
...
C       GOTO 661 FUER 1 ODER 2 TEILCHEN
      ...
      IF((ABS(OPWERT(NZOPER+2,LL)) + ABS(B2)).LT.1.E-17) GOTO 661
c     IF( ABS(B1/B2).GT.. 001) GOTO 163
   65 EBIN1(KP) = EBIN1(KP) + OPWERT(NZOPER+2,KK)
 661  CONTINUE
      EBIN1(KP) = EBIN1(KP)/ REAL(MMP)
      ...
      FAKK=REAL(MLAD(1,K)*MLAD(2,K))*E2R0*WNORM(KP)*SQRT(REDM(K))
     1  *GAMMA(2*M2)*.5
      DO 66 M=1,MMK
      DO 66 N=1,MMK1
...
209   IF (IDRU.LT.1) GOTO 210
      WRITE(NOUT,*)' NORM UND HAMILTONIAN VOR DIAGONALISATION (full)'
      CALL SCHEMA(DM(1,1,2,1),NX,NX,NDIMD,5)
      CALL SCHEMA(DM(1,1,2,2) ,NX,NX,NDIMD,5)
      WRITE(NOUT,*)' NORM UND HAMILTONIAN VOR DIAGONALISATION (f1-f2)'
      CALL SCHEMA(DM(1,1,1,1),NX,NX,NDIMD,5)
      CALL SCHEMA(DM(1,1,1,2) ,NX,NX,NDIMD,5)

      IF(IDRU.LT.3)GOTO 210
C     LAPACK-AUFRUF
       CALL DSYEVX(JOBZ,RANGE,UPLO,NX,DM(1,1,2,1),NDIMD,VL,VU,Il,IU,
...
c      MATOUN=29
c      OPEN(UNIT=29,FILE='MATOUTN',STATUS='UNKNOWN',FORM='FORMATTED')
      WRITE(MATOUN,'(I8)') MMM
      WRITE(MATOUN,'(E20.14)')
...
c      CALL SCHEMA (H,NX,NX,NDIMD,5)

c      OPEN(UNIT=19,FILE='MATOUT',STATUS='UNKNOWN',FORM='FORMATTED')
c      WRITE(19,'(I8)') MMM
c      WRITE(19,'(E20.14)')
c     *  ((EN(N,M),N=1,MMM),M=1,MMM)
c      WRITE(19,'(E20.14)')
c     *  ((H(N,M),N=1,MMM),M=1,MMM)     
c      CLOSE(UNIT=19,STATUS='KEEP')

c      OPEN(UNIT=29,FILE='MATOUTB',STATUS='REPLACE',
c     *   FORM='UNFORMATTED')
      WRITE(29) 
     * ((EN(N,M),N=1,MMM),M=1,MMM),
...
C     LAPACK-AUFRUF
      ...
      ABSTOL=0.00001 
      LWORK=8*NDIMD
c      CALL DSYEV(JOBZ,UPLO,NX,H,NDIMD,EIG,FV1,LWORK,IERR)
